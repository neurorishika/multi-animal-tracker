...
**Scope:** Rename project from "multi-animal-tracker" / "MAT" / "multi_tracker" to "HYDRA" (Holistic YOLO-based Detection Recognition and Analysis Suite)

## Naming Map

| Old | New | Context |
|-----|-----|---------|
| `multi-animal-tracker` | `hydra-suite` | pip package name, conda env base, GitHub repo, platformdirs app name |
| `multi_tracker` | `hydra_suite` | Python package (imports) |
| `multi_tracker.mat` | `hydra_suite.tracker` | Main tracker app module |
| `mat` | `hydra` | CLI entry point |
| `MAT` | `HYDRA` | Acronym in docs/comments |
| `MAT_DATA_DIR` / `MAT_CONFIG_DIR` | `HYDRA_DATA_DIR` / `HYDRA_CONFIG_DIR` | Env var overrides |
| `multi-animal-tracker` (conda) | `hydra` / `hydra-mps` / `hydra-cuda` / `hydra-rocm` | Conda environment names |
| `neurorishika/multi-animal-tracker` | `neurorishika/hydra-suite` | GitHub repo URL |
| `~/...multi-animal-tracker/` | `~/...hydra-suite/` | User data/config dirs |
...
```
src/multi_tracker/     -->  src/hydra_suite/
src/multi_tracker/mat/ -->  src/hydra_suite/tracker/
```
...
- Remove obsolete SVGs: `multianimaltracker.svg` and any other old brand files
...
- ~149 occurrences of `multi-animal-tracker` in config/docs
...
1. Rename directory `src/multi_tracker/` to `src/hydra_suite/` (including `mat/` to `tracker/`)
2. Remove old brand SVGs (`multianimaltracker.svg`, etc.)
3. Global find-replace across all .py files: `multi_tracker.mat` to `hydra_suite.tracker`, then `multi_tracker` to `hydra_suite`
