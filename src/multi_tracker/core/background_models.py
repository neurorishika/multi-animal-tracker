    ...
    def prime_background(self, cap):
        ...
        for idx in idxs:
            ...
            if ROI_mask is not None:
                roi_resized = cv2.resize(ROI_mask, (gray.shape[1], gray.shape[0]), interpolation=cv2.INTER_NEAREST) if resize_f != 1.0 else ROI_mask
    ...
    def update_and_get_background(self, gray, roi_mask, tracking_stabilized):
        ...
        if roi_mask is not None:
            ...
            self.lightest_background[roi_mask_bool] = np.maximum(
                self.lightest_background[roi_mask_bool], gray.astype(np.float32)[roi_mask_bool]
            )
        else:
            self.lightest_background = np.maximum(self.lightest_background, gray.astype(np.float32))

        if p.get("ENABLE_ADAPTIVE_BACKGROUND", True) and self.adaptive_background is not None:
